...
Run `pnpm --filter @cloudblocks/engine-spike test` from the monorepo root to verify the fixture. No production renderer or schema change is part of the comparison. Two small production compatibility fixes allow the existing root-only Front Door resource to import and display; see `REVIEW.md`.
