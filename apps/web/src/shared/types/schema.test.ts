...
describe('schema utilities', () => {
  ...
  it('migrates legacy external actors without position', () => {
    ...
    expect(result[0].architecture.externalActors?.[0]?.position).toEqual({ x: -5, y: 0, z: 0 });
  });
  ...
  it('createBlankArchitecture returns the expected default structure', () => {
    ...
    expect(architecture).toEqual({
      ...
      externalActors: [
        { id: 'ext-browser', name: 'Client', type: 'browser', position: { x: -5, y: 0, z: 0 } },
        {
          id: 'ext-internet',
          name: 'Internet',
          type: 'internet',
...
describe('migrateExternalActorsToBlocks', () => {
  ...
  it('uses default position when actor position is missing', () => {
    ...
    expect(result).toHaveLength(1);
    expect(result[0].position).toEqual({ x: -5, y: 0, z: 0 });
  });
});
