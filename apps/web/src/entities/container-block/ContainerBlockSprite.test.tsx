...
import { ContainerBlockSprite } from './ContainerBlockSprite';
...
vi.mock('./ContainerBlockSvg', () => ({
  ContainerBlockSvg: (props: Record<string, unknown>) => (
    <div
      data-testid="container-svg"
...
vi.mock('./containerBlockFaceColors', () => ({
  getContainerBlockFaceColors: (container: { type: string }) => {
...
describe('PlateSprite', () => {
  ...
  it('renders with correct position styles', () => {
    ...
    const { container } = render(
      <ContainerBlockSprite container={containerBlock} screenX={111} screenY={222} zIndex={3} />,
    );
  ...
  it.each([
    ['region', makeNetworkPlate(), 'Network ContainerBlock', '#6366F1'],
    ['subnet', makeSubnetPlate(), 'Subnet', '#6366F1'],
  ] as const)(
    'renders correct PlateSvg for %s',
    (_, container, expectedLabel, expectedTopColor) => {
      render(<ContainerBlockSprite container={container} screenX={0} screenY={0} zIndex={1} />);

      const svg = screen.getByTestId('container-svg');
  ...
  it('falls back to default PlateSvg for unknown container type values', () => {
    ...
    render(<ContainerBlockSprite container={strangePlate} screenX={0} screenY={0} zIndex={1} />);
    const svg = screen.getByTestId('container-svg');
    expect(svg).toBeInTheDocument();
    expect(svg).toHaveAttribute('data-top-face-color', '#6366F1');
  });

  it('click selects the container', async () => {
    const user = userEvent.setup();
    const container = makeNetworkPlate();
    render(<ContainerBlockSprite container={container} screenX={0} screenY={0} zIndex={1} />);

    await user.click(screen.getByRole('button', { name: `Container: ${container.name}` }));
  ...
  it('uses worldSizeToScreen for button dimensions', () => {
    const container = makeSubnetPlate();
    render(<ContainerBlockSprite container={container} screenX={0} screenY={0} zIndex={1} />);
    ...
    const button = screen.getByRole('button', { name: `Container: ${container.name}` });
  ...
  it('moves container position from drag deltas and scene zoom', () => {
    const containerBlock = makeNetworkPlate();
    const { container } = render(
    ...
    const target = container.querySelector('.container-sprite') as HTMLElement;
    ...
    expect(moveNodePositionMock).toHaveBeenCalledWith(containerBlock.id, 0, 0);
  });
  ...
  it('ignores click while dragging', async () => {
    ...
    const target = screen
      .getByRole('button', { name: `Container: ${container.name}` })
      .closest('.container-sprite') as HTMLElement;
  ...
  it('cleans up draggable on unmount', () => {
    ...
    const { unmount } = render(
      <ContainerBlockSprite container={container} screenX={0} screenY={0} zIndex={1} />,
    );
  ...
  it('resets dragging after end timeout and allows click selection', () => {
    ...
    const button = screen.getByRole('button', { name: `Container: ${container.name}` });
    const target = button.closest('.container-sprite') as HTMLElement;
  ...
  it('removes dropping class after animation end on drag release', () => {
    ...
    const sprite = screen
      .getByRole('button', { name: `Container: ${container.name}` })
      .closest('.container-sprite') as HTMLElement;
    const image = sprite.querySelector('.container-img') as HTMLElement;
  ...
  it('applies diff classes for added, modified, and removed states', () => {
    ...
    useUIStore.setState({ diffMode: true, diffDelta: makeDelta('added', 'container-added') });
    const { container, rerender } = render(
      <ContainerBlockSprite
        container={{ ...makeNetworkPlate(), id: 'container-added' }}
        screenX={0}
        screenY={0}
        zIndex={1}
      />,
    ...
    useUIStore.setState({ diffMode: true, diffDelta: makeDelta('modified', 'container-modified') });
    rerender(
      <ContainerBlockSprite
        container={{ ...makeNetworkPlate(), id: 'container-modified' }}
        screenX={0}
        screenY={0}
        zIndex={1}
      />,
    ...
    useUIStore.setState({ diffMode: true, diffDelta: makeDelta('removed', 'container-removed') });
    rerender(
      <ContainerBlockSprite
        container={{ ...makeNetworkPlate(), id: 'container-removed' }}
        screenX={0}
        screenY={0}
        zIndex={1}
      />,
  ...
  it('snaps on drag end and plays sound when unmuted', () => {
    ...
    expect(moveNodePositionMock).toHaveBeenCalledWith('container-snap', 0.8, 1.4);
  ...
  it('snaps on drag end without sound when muted', async () => {
    ...
    expect(moveNodePositionMock).toHaveBeenCalledWith('container-muted', 0.5, 0.5);
