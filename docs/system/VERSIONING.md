...
This document defines how CloudBlocks documentation distinguishes between **current implementation** (Phase 1) and **future architecture compiler vision** (v1.x), ensuring readers always know what exists today versus what is planned.
...
1. **Contributors and users of the current Phase 1 MVP** — who need accurate descriptions of what the code actually does
...
### Phase 1 — CloudBlocks Builder (Current Implementation)

The working MVP. Everything described as Phase 1 **exists in the codebase and can be verified**.

| Aspect | Phase 1 Reality |
|--------|-------------|
...
**Canonical sources for Phase 1 behavior:**
...
### Phase 2–Phase 4 — Visual Polish, Code Generation, Workspace Management

Near-term milestones building on the Phase 1 foundation. See `docs/concept/ROADMAP.md` for details.

| Version | Key Additions |
|---------|--------------|
| **Phase 2** | Drag-and-drop, keyboard shortcuts, camera controls |
| **Phase 3** | Terraform code generation (client-side, Azure-first) |
| **Phase 4** | Multi-workspace, import/export, template system |
...
### Rule 1: Phase 1 Documents Must Match Code Exactly

Any document describing Phase 1 behavior must be verifiable against the canonical source files. If the code says `category: 'compute'`, the docs must say `category`, not `type`. If the code only supports `dataflow` connections, the docs must not list `network` or `event` as available.
...
```markdown
> **Phase 6+**: This feature is part of the Architecture Compiler vision and is not yet implemented.
```
...
The domain model (types, field names, relationships) is defined in `DOMAIN_MODEL.md` and implemented in `apps/web/src/shared/types/index.ts`. All other documents must use the same field names and type definitions. If a discrepancy is found, `index.ts` is the source of truth for Phase 1, and `DOMAIN_MODEL.md` is the source of truth for the model specification.
...
| Phase 1 TypeScript types | `index.ts` | `apps/web/src/shared/types/` |
...
| `TUTORIALS.md` (1–5) | `docs/guides/` | Step-by-step guides using Phase 1 features | Running application |
...
|----------|----------|-------|-------|
| `ARCHITECTURE.md` | `docs/concept/` | System architecture overview | Sections on Phase 1 state + Architecture Compiler vision |
| `PRD.md` | `docs/concept/` | Product requirements | Covers Phase 1 deliverables + future vision |
...
| `rules.md` | `docs/engine/` | Validation rule framework | Partial — Phase 1 rules exist, extensible framework is v1.x |
...
- [ ] Connection rules follow the initiator model (database/storage are receiver-only in Phase 1)
...
| Label | Meaning |
|-------|---------|
| **Phase 1** | Exists in code today. Verified against `index.ts` and `schema.ts`. |
| **Phase 2** | Planned for next release. No code exists yet. |
| **Phase 3** | Code generation milestone. Design exists in `generator.md`. |
| **Phase 4** | Workspace management milestone. |
| **Phase 5+** | Server-side features (API-based code generation, Git integration). |
| **Phase 6+** | Architecture Compiler vision. Full DSL, multi-provider, templates. |
