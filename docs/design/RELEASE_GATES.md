...
## 7. Historical Gate Records

The completed milestone and phase checklists are preserved verbatim in [Completed Release Gates](COMPLETED_RELEASE_GATES.md). Their counts and version convention describe past checkpoints, **not** current guarantees or a release trigger. Current versioning follows [Version Alignment Policy](VERSION_POLICY.md); beta-specific readiness is tracked separately in [#1916](https://github.com/yeongseon/cloudblocks/issues/1916).

---

## 8. Cross-Cutting Gates

These gates are not tied to a specific milestone. They apply to every release once their instrumentation is in place.
