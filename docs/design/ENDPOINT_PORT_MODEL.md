...
This document defines the **Endpoint and Port** architecture introduced in Schema v4 (M22 Core Lock). It replaces the legacy stub-based connection model with a typed, directional endpoint system.

### Terminology

| Term           | Definition                                                                                                                        | Layer          |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| **Endpoint**   | A typed connection point on a node. Every node has 6 endpoints (3 semantics × 2 directions). Persisted in the architecture model. | Domain model   |
...
| **Node**       | A resource in the architecture (container or leaf). Replaces the legacy "Block" and "Plate" terms in UI.                          | Domain model   |

> **Note**: In code, the type names `ContainerNode`, `LeafNode`, `ResourceNode`, `Endpoint`, and `Connection` are canonical. The terms "Node", "Port", and "Endpoint" are used in UI strings. See §6 for the full terminology mapping.
...
Endpoint IDs are deterministic — computed from the node ID, direction, and semantic type:

```
endpoint-{nodeId}-{direction}-{semantic}
```
...
```typescript
function generateEndpointsForNode(nodeId: string): Endpoint[] {
  const endpoints: Endpoint[] = [];
  for (const direction of ['input', 'output']) {
    for (const semantic of ['http', 'event', 'data']) {
      endpoints.push({
        id: `endpoint-${nodeId}-${direction}-${semantic}`,
        nodeId,
        direction,
        semantic,
      });
...
Direction is relative to the **node**, not the connection. A connection always flows from an `output` endpoint to an `input` endpoint.
...
2. **No self-connections**: A node cannot connect to itself (validated by checking `from.nodeId !== to.nodeId`).
...
To resolve a connection's source/target nodes and type:

```typescript
function resolveConnectionNodes(conn: { from: string; to: string }): {
  sourceId: string;
  targetId: string;
  type: string;
} {
...
| --------------- | ----- | ---------------------------------- |
| Dot size (rx)   | 4px   | `STUB_DOT_RX` in `designTokens.ts` |
| Dot size (ry)   | 2.5px | `STUB_DOT_RY` in `designTokens.ts` |
| Stroke width    | 1.5px | `STUB_DOT_STROKE_WIDTH`            |
| Default opacity | 0.7   | `STUB_DOT_OPACITY`                 |
| Active opacity  | 1.0   | `STUB_DOT_ACTIVE_OPACITY`          |
...
6. Invalid ports (wrong semantic, same node, occupied) are dimmed
...
```
✅ Valid: output → input, same semantic, different node, both unoccupied
❌ Invalid: same direction, different semantic, same node, occupied target
```
...
| ----------------------------------- | --------------------------------------- | ------------------------------------------------------- |
| `Connection.sourceId` / `targetId`  | `Connection.from` / `to` (endpoint IDs) | Connections reference endpoints, not nodes              |
| `ConnectionType` enum               | `EndpointSemantic` enum                 | `dataflow`→`data`, `async`→`event`, `http`→`http`       |
| `sourceStub` / `targetStub` (index) | Deterministic endpoint IDs              | No more index-based stub references                     |
| No endpoint concept                 | `Endpoint[]` in model                   | Endpoints are first-class model entities                |
| `ExternalActor`                     | Folded into nodes                       | External actors are now nodes with special resourceType |
...
| ----------------- | --------------- | -------------------------------------------------------------- |
| `ResourceNode`    | **Node**        | Generic reference to any element on canvas                     |
| `ContainerNode`   | **Container**   | A node that holds child nodes (deprecated: "Plate")            |
| `LeafNode`        | **Node**        | A leaf resource (deprecated: "Block")                          |
| `Endpoint`        | **Endpoint**    | Connection point on a node (deprecated: "Stub")                |
| Port (visual dot) | **Port**        | The rendered visual dot on block faces (deprecated: "UI Stub") |
| `Connection`      | **Connection**  | A link between two endpoints                                   |

> **Important**: These are UI-only terms. Code should continue using the TypeScript type names (`ContainerNode`, `LeafNode`, `Endpoint`, etc.).
...
export interface ArchitectureModel {
  ...
  name: string;
  version: string;
  nodes: ResourceNode[]; // All nodes (containers + resources)
  endpoints: Endpoint[]; // All endpoints (auto-generated per node)
...
1. For every node in `nodes`, exactly 6 endpoints exist in `endpoints` (3 semantics × 2 directions).
...
6. `endpoint.nodeId` references a valid node in `nodes`.
