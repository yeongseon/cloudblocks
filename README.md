...
| Semantics   | Visual only  | Typed model; Azure export blocks unsupported services instead of silently substituting them |
...
- ⚡ **Terraform starter export** — Generate illustrative Terraform for learning. Azure rejects unsupported services rather than silently substituting another service; supported output can still require manual configuration. [Review the mapping limits](docs/user-guide/provider-support.md) before using any output.
